...
class MainWindow(QtGui.QMainWindow):
    ...
    def set_current_frame(self, frame):
        ...
        r = seq[frame][0]
        self.sheet.set_highlight_rect(r)
    ...
    def on_new(self):
        (filename,ok)=QtGui.QFileDialog.getOpenFileNameAndFilter(self,"Sprite Image Sheet",os.getcwd(),"*.png","*.png")
        if ok:
            self.seq_dict = {}
    ...
    def on_open(self):
        (filename,ok)=QtGui.QFileDialog.getOpenFileNameAndFilter(self,"Load Sprite",os.getcwd(),"*.json","*.json")
        if ok:
            ...
            for seq in root.get('Sequences'):
                ...
                for frame in seq.get('Frames'):
                    r=frame.get('Rect')
                    d=frame.get('Duration')
                    frames.append((tuple(r),d))
